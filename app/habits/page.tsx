...
import { habitsService, HabitWithLogs } from '@/lib/habits.service'
...
export default function HabitsPage() {
  ...
  const toggleHabit = async (id: string, completedToday: boolean) => {
    try {
      if (completedToday) {
    ...
    <div className="flex h-screen bg-black text-white overflow-hidden">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-black border-r border-white/10 transform transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
            ...
            <Link href="/goals" className="flex items-center space-x-3 px-4 py-3 rounded-xl hover:bg-white/5 text-gray-400 hover:text-white transition-colors">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <Link href="/habits" className="flex items-center space-x-3 px-4 py-3 rounded-xl bg-[#A855F7]/10 text-[#A855F7] font-semibold">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {/* Today's Progress */}
          {totalCount > 0 && (
            <div className="bg-gradient-to-br from-purple-900/50 to-purple-800/30 rounded-xl p-6 border border-purple-700/30 mb-6">
              ...
              {habits.map(habit => (
                <div
                  ...
                  className={`bg-zinc-900/50 rounded-xl border p-4 transition-all ${
                    habit.completedToday ? 'border-green-500/50 bg-green-500/5' : 'border-zinc-800'
                  }`}
                >
                  <div className="flex items-center gap-4">
                    <button
                      onClick={() => toggleHabit(habit.id, habit.completedToday)}
                      className={`w-12 h-12 rounded-full border-2 flex items-center justify-center transition-all flex-shrink-0 ${
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className={`font-semibold ${habit.completedToday ? 'text-gray-400' : 'text-white'}`}>
                          {habit.title}
                        </h3>
                      </div>
                      {habit.description && (
                  ...
                  {(['daily', 'weekly', 'monthly'] as const).map(f => (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
                        habitFrequency === f ? 'bg-purple-600 text-white' : 'bg-zinc-800 text-gray-400'
                      }`}
                    >
                ...
                <div className="flex gap-2">
                  {['#F59E0B', '#10B981', '#8B5CF6', '#EF4444', '#3B82F6', '#EC4899'].map(color => (
                    <button
                      ...
                      className={`w-8 h-8 rounded-full ${habitColor === color ? 'ring-2 ring-white ring-offset-2 ring-offset-zinc-900' : ''}`}
