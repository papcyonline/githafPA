...
export function RecentActivityFeed({ activities }: RecentActivityFeedProps) {
  ...
  const getActionIcon = (actionType: string, entityType: string) => {
    ...
    const bgColors: Record<string, string> = {
      ...
      note: 'bg-blue-500/20',
      reminder: 'bg-amber-500/20',
      recording: 'bg-green-500/20',
      goal: 'bg-emerald-500/20',
      habit: 'bg-orange-500/20',
      event: 'bg-cyan-500/20',
    }

    return (
      <div className={`w-8 h-8 rounded-full ${bgColors[entityType] || 'bg-gray-500/20'} flex items-center justify-center`}>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-gray-300">
                ...
                {activity.entity_title && (
                  <span className="font-medium text-white"> "{activity.entity_title}"</span>
                )}
              </p>
